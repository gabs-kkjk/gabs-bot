...
export default {
  ...
  handle: async ({ userLid, args, fullMessage, sendReply }) => {
    ...
    const receiver = `${mention[1]}@lid`;
    if (receiver === userLid) return sendReply("💜 Você não pode doar para si mesmo.");

    const users = getUsers();
    const from = getUser(users, userLid);
