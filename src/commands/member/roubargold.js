...
export default {
  ...
  handle: async ({ userLid, fullMessage, sendReply }) => {
    const mention = fullMessage.match(/@(\d{5,})/);
    if (!mention) return sendReply("💜 Marque quem você quer roubar.");

    const target = `${mention[1]}@lid`;
    if (target === userLid) return sendReply("💜 Você não pode roubar você mesmo.");

    const users = getUsers();
    const thief = getUser(users, userLid);
